...
trait BasicHttpService extends Directives {
   ...
   */
  def route: Route = {
    // // zevin: [logging] controller_gets_request
  ...
  protected def assignId = HeaderDirectives.optionalHeaderValueByName(OW_EXTRA_LOGGING_HEADER) flatMap { headerValue =>
    ...
    extract { req =>
      ...
      // zevin: here, the tid is generated by parsing the http request header from nginx and we got TransactionId
  ...
  protected def logRequestInfo(req: HttpRequest)(implicit tid: TransactionId): LogEntry = {
    val timestamp = System.currentTimeMillis() // zevin: [TimeStamp] controller_gets_request
    ...
    LogEntry(s"[$tid] $m $p $q. zevin: [TimeStamp] controller_gets_request. ${timestamp}ms", l)
  }
